<div x-data="song" class="relative text-left text-slate-700 space-y-1.5">
    <div class="space-y-2">
        <input wire:model.live="value" @focusin="focusIn" @focusout="focusOut" type="{{ $type }}" id="{{ $trigger }}" placeholder="{{ $placeholder }}" class="transition-all duration-300 ease-in-out w-full h-8 border border-gray-200 rounded-lg pl-3.5 pr-10 pt-6 pb-5 outline-none focus:border-blue-500 focus:ring focus:ring-blue-500/30">
    ...
    @if($selectedSong)
        <div class="relative flex gap-4 w-full bg-gray-200 rounded-lg p-3.5">
    ...
    @if($value)
        <div x-show="dropdown" class="absolute rounded-lg h-[300px] w-full bg-gray-200 overflow-y-scroll overflow-x-hidden z-10 @if($selectedSong) top-12 @else inset-y-full @endif">
